...
class JaxeInterp {
	...
	public function expr(e:JaxeExpr):Dynamic {
		...
		switch (e) {
			...
			// --- MACRO INTEGRATION UPDATES START HERE ---
			case EMethod(name, type, args, body, isOverride, isStatic):
				...
				if (targetObj != null && !isStatic) {
					var originalName = "_original_" + cleanName;
					var originalMethod:Dynamic = null;

					// 1. Look for the macro-generated hidden method
					if (Reflect.hasField(targetObj, originalName) || Reflect.getProperty(targetObj, originalName) != null) {
						originalMethod = Reflect.getProperty(targetObj, originalName);
					} 
					// 2. Fallback to standard reflection
					else if (Reflect.hasField(targetObj, cleanName) || Reflect.getProperty(targetObj, cleanName) != null) {
						originalMethod = Reflect.getProperty(targetObj, cleanName);
					}

					if (originalMethod != null) {
						superMethods.set(cleanName, originalMethod);
					}

					// 3. Register with our new Proxy Manager
					jaxe.JaxeProxyManager.addInstanceProxy(targetObj, cleanName, this);

					// Fallback for fields manually containing function closures
					var hookName = cleanName;
					if (Reflect.hasField(targetObj, "scripted_" + cleanName)) hookName = "scripted_" + cleanName;
				...
				return null;
			// --- MACRO INTEGRATION UPDATES END HERE ---
